...
- Track-change updates accept a progress snapshot only when it matches the new-track signature (`new_track_signature`): raw position ≤ 10s (`NEW_TRACK_MAX`), or a duration different from the pre-change track's, or no prior progress (startup mid-track). Required because the old track's prints keep *advancing* after the metadata flip (auto-advance lag) and its final print lingers unresolved across the gap — Python's universal-newline reader cannot return a `\r`-terminated line until the next stdout byte arrives (the original wrong-timer bug). On timeout the update is sent without timestamps; the worker's idle tick re-sends with a timer once a fresh snapshot matches the signature and isn't identical to the rejected prior (`timer_pending`/`pending_prior`). Resume-restore keeps the advancement rule: raw position must change between two fresh snapshots (frozen = not actually playing). The identity-skip in `PresenceBridge` also compares a `has_timer` marker so timer-less → timed transitions re-send.
